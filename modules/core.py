...
class ParallelModule(nn.DataParallel):
    """
    A DataParallel wrapper that forwards unknown attributes to the inner .module.
    This lets your custom methods (clip_layer, encode_from_tokens, etc.) still work.
    """
...
def load_model(ckpt_filename, vae_filename=None):
    """
    Load UNet, CLIP, VAE, wrap in ParallelModule for multi-GPU, and return
    the StableDiffusionModel.
    """
    # 1) load the raw components
    ...
    # 2) wrap them in the high-level container
    ...
    # 3) move raw models to GPU0
    sd_model.unet.model.to(device)
    sd_model.clip.cond_stage_model.to(device)
    if sd_model.vae is not None:
        sd_model.vae.first_stage_model.to(device)

    # 4) wrap each sub-module in our ParallelModule
    sd_model.unet.model = ParallelModule(sd_model.unet.model)
    sd_model.clip.cond_stage_model = ParallelModule(sd_model.clip.cond_stage_model)
    if sd_model.vae is not None:
        sd_model.vae.first_stage_model = ParallelModule(sd_model.vae.first_stage_model)

    # 5) re-clone your LoRA wrappers so they point at the parallel modules
    sd_model.unet_with_lora   = sd_model.unet.clone()
    sd_model.clip_with_lora   = sd_model.clip.clone()
